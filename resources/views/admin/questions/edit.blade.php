                    ...
                    <form action="{{ route('admin.questions.update', $question) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="category" id="category" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                <option value="Not categorized" @selected(old('category', $question->category) == 'Not categorized')>Not categorized</option>
                                <option value="Basic Care and Comfort" @selected(old('category', $question->category) == 'Basic Care and Comfort')>Basic Care and Comfort</option>
                                <option value="Health Promotion and Maintenance" @selected(old('category', $question->category) == 'Health Promotion and Maintenance')>Health Promotion and Maintenance</option>
                                <option value="Management of Care" @selected(old('category', $question->category) == 'Management of Care')>Management of Care</option>
                                <option value="Pharmacological and Parenteral Therapies" @selected(old('category', $question->category) == 'Pharmacological and Parenteral Therapies')>Pharmacological and Parenteral Therapies</option>
                                <option value="Physiological Adaptation" @selected(old('category', $question->category) == 'Physiological Adaptation')>Physiological Adaptation</option>
                                <option value="Reduction of Risk Potential" @selected(old('category', $question->category) == 'Reduction of Risk Potential')>Reduction of Risk Potential</option>
                                <option value="Safety and Infection Control" @selected(old('category', $question->category) == 'Safety and Infection Control')>Safety and Infection Control</option>
                            </select>
                        </div>
